...
def index_of_point(mapData, Xp):
    ...
    Data = mapData.data
    index = int(	(floor(old_div((Xp[1]-Xstarty),resolution)) *
                  width)+(floor(old_div((Xp[0]-Xstartx),resolution))))
    return index


def point_of_index(mapData, i):
    y = mapData.info.origin.position.y + \
        (old_div(i,mapData.info.width))*mapData.info.resolution
    x = mapData.info.origin.position.x + \
        (i-(old_div(i,mapData.info.width))*(mapData.info.width))*mapData.info.resolution
    return array([x, y])
# ________________________________________________________________________________
...
def informationGain(mapData, point, r):
    ...
    r_region = int(old_div(r,mapData.info.resolution))
    init_index = index-r_region*(mapData.info.width+1)
    for n in range(0, 2*r_region+1):
        start = n*mapData.info.width+init_index
        end = start+2*r_region
        limit = ((old_div(start,mapData.info.width))+2)*mapData.info.width
...
def gridValue(mapData, Xp):
    ...
    index = (floor(old_div((Xp[1]-Xstarty),resolution))*width) + \
        (floor(old_div((Xp[0]-Xstartx),resolution)))
